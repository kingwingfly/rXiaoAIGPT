...
- `tools.rs` — the `brain::Tool` impls: `play_music` (local library first, NetEase on a miss; `source: "netease"` skips local), `stop`, `set_volume`, `tell_story`. Adding a capability means one `Tool` impl and one `register` call.
...
- `main.rs` — builds one `Arc<MusicIndex>` shared between the router and `LocalSource` (a second index could disagree about what exists), mounts `NeteaseSource::router()`, registers the tools, and starts `brain::Agent`. A missing `DEEPSEEK_API_KEY` fails here, by name, rather than as a 401 from the first thing anyone says.
